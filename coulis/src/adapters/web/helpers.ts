...
export const createClassName = (input: string) => {
	// Content hashing based on FNV-1a algorithm:
	const FNVOffsetBasis = 2_166_136_261;
	const FNVPrime = 16_777_619;
	let hashedValue = FNVOffsetBasis;

	for (let index = 0; index < input.length; index++) {
		// codePointAt() is slower than charCodeAt() for ASCII content (CSS values are always ASCII)
		// eslint-disable-next-line unicorn/prefer-code-point
		hashedValue ^= input.charCodeAt(index);
		hashedValue = Math.imul(hashedValue, FNVPrime);
	}
	 ...
	 */
	return `c${(hashedValue >>> 0).toString(16)}`;
};
